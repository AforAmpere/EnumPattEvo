...
queue<pair<int,int>>& linked(int trans, int newstate)
{
	...
	q.push({linkedtrans[trans],1-newstate});
	return q;
}
...
unsigned long long int check_frontend(RulespaceRep& rulespace, CellArray& arr)
{
	...
	for(int side=0;side<4;side++)
	{
		...
		{
			nxt=arr.cells[(side&1)?c:bx][(side&1)?by:c];
			if(nxt!=state)
			{
				queue[0]=queue[1];
				queue[1]=queue[2];
...
RulespaceRep maxspace = rulespace_from_rules(vector<RuleRep>({parse_rule("B/S8"),parse_rule("B12345678/S012345678")}));