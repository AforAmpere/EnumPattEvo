...
unordered_map<string, string> help_map()
{
    ...
    add_keys_message_pair<string>(helpmap, {"h","help"}, 
"-h/--help [option1, option2,...]\n\n\
...
Pulls up info for -p and -i commands."
    ...
    add_keys_message_pair<string>(helpmap, {"p","pattern"}, 
"-p/--pattern {RLE1} [, RLE2,...]\n\n\
...
Confines the rulespace to that matching the input file (example.txt)'s specifications, and also sets the pattern in the file as the starting pattern."
    ...
    add_keys_message_pair<string>(helpmap, {"b","bounding_box"},
"-b/--bounding_box {width1, height1}, [width2, height2, width3, height3,...]\n\n\
...
Sets the bounding boxes for the first two patterns to 5x5 and 4x7."
    ...
    add_keys_message_pair<string>(helpmap, {"g","generations"},
"-g/--generations {depth1}, [depth2, depth3,...]\n\n\
...
Sets the generation depth for the first pattern at 100, and the second at 3."
    ...
    add_keys_message_pair<string>(helpmap, {"r","rulespace"},
"-r/--rulespace {rule1} [, rule2,...]\n\n\
...
For INT, allows rules with anything in B23/S1c, but forces B2a transition, and everything else is off."
    ...
    add_keys_message_pair<string>(helpmap, {"t","target"},
...
Sets the first pattern's target to the LWSS, and the second to an empty pattern."
    ...
    add_keys_message_pair<string>(helpmap, {"e","evolve"}, 
"-e/--evolve [flag1, flag2, ...]\n\n\
...
Allows said reporting to happen for the third pattern, but no others (if there are 4 starting patterns)."
    ...
    add_keys_message_pair<string>(helpmap, {"m","move"},
"-m/--move [flag1, flag2, ...]\n\n\
...
Allows said reporting to happen for the third pattern, but no others (if there are 4 starting patterns)."
    ...
    add_keys_message_pair<string>(helpmap, {"nosymm"},
"--nosymm [flag1, flag2, ...]\n\n\
...
Allows said reporting to happen for the third pattern, but no others (if there are 4 starting patterns)."
    ...
    add_keys_message_pair<string>(helpmap, {"noexp"}, 
"--noexp [flag1, flag2, ...]\n\n\
...
compatible with patterns that also use -t. This flag will be ignored for those patterns (currently)."
    ...
    add_keys_message_pair<string>(helpmap, {"i","input_evo"},
"-i/--input_evo {file1} [, file2,...]\n\n\
...
Confines the rulespace to that matching the input file (example.txt)'s specifications."
    ...
    add_keys_message_pair<string>(helpmap, {"f","file"}, 
"-f/--file {file}\n\n\
...
Directs results to newoutput.txt."
    ...
    add_keys_message_pair<string>(helpmap, {"a","append"},
"-a/--append\n\n\
If on, sets output to append to results file instead of resetting it every time."
); 
    add_keys_message_pair<string>(helpmap, {"s","save"}, 
"-s/--save {file}\n\n\
...
Saves state to savedata.txt."
    ...
    add_keys_message_pair<string>(helpmap, {"l","load"},
"-l/--load {file}\n\n\
...
Starts at [1/n1], [6/n2], [7/n3], [31/n4]."
    ...
    add_keys_message_pair<string>(helpmap, {"5s"},
"--5s [foldername]\n\n\
...
Loads the speeds from the 5s database, and turns on 5s mode."
    ...
    add_keys_message_pair<string>(helpmap, {"mp"},
"--mp {period}\n\n\
Sets a minimum period that results can be output as.\n\n\
Example: --mp 3\n\
No oscillators or ships under p3 will be reported."
);
    add_keys_message_pair<string>(helpmap, {"filter"},
"--filter {stringofineqs1} [, stringofineqs2,...]\n\n\
...
For the second no filter is specified. For the third, it must move by 3 or more in the y direction."
    ...
    add_keys_message_pair<string>(helpmap, {"prune"},
"--prune {stringofineqs1} [, stringofineqs2,...]\n\n\
...
For the second no pruning is specified. For the third, it must have moved by -1 or more in the y direction from the starting position at all times."
    ...
    add_keys_message_pair<string>(helpmap, {"confslid"},
"--confslid {RLE1} [, RLE2,...]\n\n\
...
Forces the pattern to be contained within the specified shape, which moves around with the pattern."
    ...
    add_keys_message_pair<string>(helpmap, {"sliceslid"},
"--sliceslid {RLE1} [, RLE2,...]\n\n\
...
Only checks if the final pattern equals the target inside the pattern in the RLE."
    ...
    add_keys_message_pair<string>(helpmap, {"confstat"},
"--confstat {RLE1, offx, offy} [, RLE2, offx, offy...]\n\n\
Forces the pattern to be contained in the region specified in the RLE provided. This means that all on cells must be in areas where the mask RLE also has \
...
Forces the pattern to be contained within the specified shape, the mask is offset left by 1 and up by 2."
    ...
    add_keys_message_pair<string>(helpmap, {"slicestat"},
"--slicestat {RLE1, offx, offy} [, RLE2, offx, offy...]\n\n\
Pattern matching is only checked within the region specified by the RLE provided. This means that the desired end pattern (either the start for non -t runs, \
or whatever the target is for -t runs) is only checked to equal the current if the cells in the RLE are on at that location. This can be used for searches \
...
Only checks if the final pattern equals the target inside the pattern in the RLE, the mask is offset left by 1 and up by 2."
    ...
    add_keys_message_pair<string>(helpmap, {"report"},
"--report {interval}\n\n\
...
Report on progress every 10000 branches, and output to a file the latest results."
    ...
    add_keys_message_pair<string>(helpmap, {"rand"},
"--rand {seed}\n\n\
...
{
    ifstream ssssstypes[4];
    ssssstypes[0].open(folder+"/"+"diagonal.sss");
    ssssstypes[1].open(folder+"/"+"orthogonal.sss");
    ssssstypes[2].open(folder+"/"+"oblique.sss");
    ssssstypes[3].open(folder+"/"+"oscillator.sss");
...
void parse_input(int argc, char** argv, bool loading=false)
{
    ...
    for(int i=1; i<argc; i++)
    {
        ...
        //Loading from file
        if(!loading && opts.back().size()>1&&(opts.back()[0]=="-l"||opts.back()[0]=="--load"))
        {
    ...
    for(auto v:opts)
    {
        ...
        else if(v[0]=="-b" || v[0]=="--bounding_box")
        {
            for(int i=1; i<v.size();i++)
            {
                if(i%2==1)
        ...
        else if(v[0]=="--mp" || v[0]=="--minimum_period")
        {
            OptionState::minperiod=stoi(v[1]);
        }

        else if(v[0]=="-f" || v[0]=="--file")
        {
        ...
        else if(v[0]=="--prune")
        {
            for(int i=1; i<v.size();i++)
            {
                OptionState::prune.push_back(NumericalConditions("prune"));
                ...
                for(int j=0;j<q.first.size();j++)
                {
                    ...
                    if(q.second[j]==3)
                    {
                        n2=0;
                        auto b = bounds(q, z, j);
                        for (string s: OptionState::prune[i-1].conditionlist)
                        {
                            if(q.first[pos2]==s)
                            {
                                OptionState::prune[i-1].set_min(s,max(OptionState::prune[i-1].get_min(s),b.first));
                                OptionState::prune[i-1].set_max(s,min(OptionState::prune[i-1].get_max(s),b.second));
                            }
                        }
                        z=j;
                    }
        ...
        else if(v[0]=="--filter")
        {
            for(int i=1; i<v.size();i++)
            {
                OptionState::filter.push_back(NumericalConditions("prune"));
                if(v[i].size()==0) continue;
                auto q = chunk(v[i]);
                int n2 = 0;
                int pos2=0;
                ...
                for(int j=0;j<q.first.size();j++)
                {
                    ...
                    if(q.second[j]==3)
                    {
                        n2=0;
                        auto b = bounds(q, z, j);
                        for (string s: OptionState::filter[i-1].conditionlist)
                        {
                            if(q.first[pos2]==s)
                            {
                                OptionState::filter[i-1].set_min(s,max(OptionState::filter[i-1].get_min(s),b.first));
                                OptionState::filter[i-1].set_max(s,min(OptionState::filter[i-1].get_max(s),b.second));
                            }
                        }
                        z=j;
                    }
...
void check_parsed()
{
    ...
    if(helpcount!=0)
    {
        if(helpcount==1)
        {
            cout<<"Options are -p/--pattern, -t/--target, -b/--bounding_box, -g/--generations, -r/--rulespace, -e/--evolution, -m/--move, -i/--input_evo, -f/--file, -a/--append, -s/--save, -l/--load, --5s, --mp, --filter, --prune, --report, --nosymm, --noexp, --confslid, --confstat, --sliceslid, --slicestat, --rand, and -h/--help at the moment. Use -h [option1, option2...] to show info on those options. -h h is an example."<<endl;
        }
        else
        {
            auto helpmessages = help_map();
            for(int i=1;i<OptionState::help.size();i++)
            {
                string message = helpmessages[OptionState::help[i]];
                if (message!="")
                {
                    cout<<message<<endl;
                }
                ...
                {
                    cout<<OptionState::help[i]<< " not recognized."<<endl;
                }
    ...
    else
    {
        ...
        OptionState::prune.resize(pattcount, NumericalConditions("prune"));
        OptionState::filter.resize(pattcount, NumericalConditions("filter"));
